...
function draw(){
  ...
  fill(50, 150, 50)
  let xElev = [ -stageV, -stageV + 100, -stageV + 1000, -stageV + 1500, -stageV + 1880, -stageV + 2000];
  let yElev = [500, 400, 500, 400, 500, 400];
  ...
  stroke(0, 0, 255);
  strokeWeight(30);
  line(xElev[0], yElev[0], xElev[1], yElev[0]);
  line(xElev[1], yElev[0], xElev[1], yElev[1]);
  line(xElev[1], yElev[1], xElev[2], yElev[1]);
  line(xElev[2], yElev[1], xElev[2], yElev[2]);
  line(xElev[2], yElev[2], xElev[3], yElev[2]);
  line(xElev[3], yElev[2], xElev[3], yElev[3]);
  line(xElev[3], yElev[3], xElev[4], yElev[3]);
  line(xElev[4], yElev[3], xElev[4], yElev[4]);
  line(xElev[4], yElev[4], xElev[5], yElev[4]);
  line(xElev[5], yElev[4], xElev[5], yElev[5]);
  line(xElev[5], yElev[5], xElev[6], yElev[5]);
  ...
  if(!keyIsPressed && facingRight ){
    image(standR[0], rightS, rise, 200, 200);   
  }
  if(!keyIsPressed && facingRight && stageV < 0){
    image(standR[0], rightS, rise, 200, 200);   
  }
  if(!keyIsPressed && facingLeft){
    image(standL[0], leftS, rise, 200, 200)
  }
...
function moveStage(){
  ...
  if(keyIsDown(LEFT_ARROW) === true) {
     ...
     facingRight = false
     if (runto.length > 0) {
    image(runto[newFrame], left, rise, 200, 200)
    }
  ...
  if(keyIsDown(RIGHT_ARROW) === true){
     ...
     facingRight = true
     if (run.length > 0) {
    image(run[currentFrame], right, rise, 200, 200)
  }     
     if (frameCount % time === 0) {
